...
public class mer_rule_Rule1011 extends mer_rule {
  public mer_rule_Rule1011() {
       this.setRuleID("Rule1011");
  ...
  public void callRule(final Session session) {
    ...
    for ( Fact<Mer> factmer : session.getWorkingMemory().getFact( Mer.class )){
       ...
       if (factmer.getFactStatus()!=FactStatus.DELETE && rules_Mer_mer_it(mer)) {
             ...
             rules_Rule1011_action(session,mer);
  ...
  public void rules_Rule1011_action(final SessionInterface it, final Mer mer) {
    String _ruleID = this.getRuleID();
    ExceptTran exTran = CalcData2.getRule1012(mer, null, null, "week", this.X, _ruleID);
